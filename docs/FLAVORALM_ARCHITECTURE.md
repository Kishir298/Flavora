...
- Not Ollama, Qwen, Llama, Mistral, Gemma, GPT, Claude, Groq, or any hosted LLM.
- Not Hugging Face pretrained weights or a renamed third-party checkpoint.
- `grep -R ollama|groq` returns zero implementation hits (the former Groq
  remote fallback was removed; see PROGRESS.md).
...
validation (loss/perplexity), per-epoch resumable `training_state.pt (intentionally untracked, 10.2M — see FLAVORALM_FINAL_AUDIT.md:87; not committed)`
...
(`{config, model_state}`), `training_state.pt (intentionally untracked, 10.2M — see FLAVORALM_FINAL_AUDIT.md:87; not committed)`, `metrics.json`
(`{step, valLoss, valPerplexity, trainLoss}`), `training_meta.json`,
`model_meta.json`, `eval.json (generated by npm run evaluate:llm; absent until run)`. No opaque checkpoints: every artifact carries
...
`assistantService.ts` is local-only: `local` (default) = FlavoraLM with
awaited `probeAvailability()` then `/intent`; unreachable/timeout/invalid/
unloaded → deterministic heuristic with an explicit `fallbackReason`
(`local-unreachable|local-timeout|local-invalid|local-unloaded`).
`heuristic` = deterministic parsing, no LLM. There is no remote provider.

## React integration + fallback

`client/src/lib/api.ts` talks only to Express. `AiStatus` always names the
actual source (`AI: FlavoraLM vX` / `AI: Heuristic (unavailable|timed out|answer invalid|starting)`).
...
- Privacy: all requests stay on the machine — there is no remote provider
  and no API key (the former Groq fallback was removed; see PROGRESS.md).
