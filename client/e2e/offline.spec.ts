 ...
 * Deterministic: navigates directly to a seeded recipe with a known
 * substitutable ingredient (`french-onion-soup` contains `butter` →
 * margarine/oil swap), instead of depending on chat ranking.
 */
test("substitution: apply then revert on a recipe detail page", async ({ page }) => {
  test.setTimeout(120_000);
  await page.goto("/recipe/french-onion-soup");
