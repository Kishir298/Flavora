...
| Inference | COMPLETE | Live `/generate` returned real model tokens (`{"ingredients":["butter","cream"],"`, 24 toks, 160 ms); not hardcoded; no Ollama/Qwen/Llama/Groq in path (grep-clean, loopback-only provider) |
...
| Ollama removed | COMPLETE | `grep ollama\|qwen\|mistral\|gemma` → only doc negations + audit policy; `llama` only as Groq remote string; generation path proven local |
